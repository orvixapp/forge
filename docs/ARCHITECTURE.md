...
- **VT state machine**: `libghostty-vt` **[DECIDIDO en Fase 2]** (Zig, C API sin dependencias, marcas OSC 133, hipervínculos, teclado Kitty, búsqueda y formatter nativos; API inestable, por eso el wrapper `proto-ghostty-vt` fija un commit). `alacritty_terminal` queda como segundo `VtEngine` opcional (`--features alacritty`) para comparar parseo y como plan B: en DEV-1 Ghostty da la mitad de latencia tecla→eco y ~28 % menos memoria por sesión (`bench/results/2026-09-13-engine-comparison.md`). Alternativa descartada: `wezterm-term`/`termwiz` (más pesado, acoplado a wezterm). Conformance: `docs/CONFORMANCE.md`.
...
- **Token Harbor** es un gateway opcional, nunca el proveedor obligatorio ni un agente ACP. Su conector oficial configura Codex, Claude Code y OpenCode y conserva copias para revertir los cambios; Forge continúa hablando ACP con el agente configurado. Encaja como perfil `openai-compatible`, junto a cualquier otro gateway. La ruta nativa del agente sigue disponible y tiene prioridad cuando el usuario no selecciona un perfil. **[VERIFICADO 2026-09-14]**
