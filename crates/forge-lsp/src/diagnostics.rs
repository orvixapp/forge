...
impl DocumentDiagnostics {
    ...
    pub fn for_line_range(&self, lines: Range<u32>) -> Vec<&Diagnostic> {
        ...
        for (_, indices) in self.line_index.range(lines) {
            for &idx in indices {
                if seen.insert(idx) {
...
impl DiagnosticStore {
    ...
    pub fn update(&self, uri: Uri, version: Option<i32>, items: Vec<Diagnostic>) {
        ...
        let mut guard = self.inner.write().expect("DiagnosticStore lock poisoned");
    ...
    pub fn for_line(&self, uri: &Uri, line: u32) -> Vec<Diagnostic> {
        let guard = self.inner.read().expect("DiagnosticStore lock poisoned");
        guard
            .get(uri)
    ...
    pub fn for_document(&self, uri: &Uri) -> Vec<Diagnostic> {
        let guard = self.inner.read().expect("DiagnosticStore lock poisoned");
        guard.get(uri).map(|doc| doc.items.clone()).unwrap_or_default()
    }
    ...
    pub fn counts_for_document(&self, uri: &Uri) -> (usize, usize, usize) {
        ...
        guard
            .get(uri)
            .map(|doc| (doc.error_count(), doc.warning_count(), doc.items.len()))
            .unwrap_or((0, 0, 0))
    }
